...
N-Gram 模型使用局部模式（卷积）进行时序预测：
1. CNN N-Gram: 多尺度 1D 卷积
2. TCN (Temporal Convolutional Network): 膨胀卷积 + 残差
3. WaveNet: 深度膨胀卷积 + 门控激活
"""
...
def save_results(
    ...
    summary_df.to_csv(output_dir / "ngram_models_summary.csv", index=False)
    print(f"\n💾 汇总结果已保存：{output_dir / 'ngram_models_summary.csv'}")

    # 保存详细指标
    with open(output_dir / "ngram_models_details.json", "w", encoding="utf-8") as f:
        details = {}
        for model_name, result in results.items():
    ...
    print(f"💾 详细指标已保存：{output_dir / 'ngram_models_details.json'}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="批量训练 N-Gram 模型")
    parser.add_argument("--panel", type=Path, default=PROCESSED_DIR / "training_panel_lite.csv")
    parser.add_argument("--output", type=Path, default=REPORTS_DIR / "ngram_models")
    ...
    try:
        ...
        model_configs = {
            "cnn_ngram": {
                ...
                "filters": 64,
                "kernel_sizes": [2, 3, 5],  # 2-gram, 3-gram, 5-gram
                "dropout": 0.3,
                "epochs": args.epochs,
