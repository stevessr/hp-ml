...
# ==================================================================================
# 👑 工业界顶级多资产【全局贝塔趋势共振自适应调节器】控制面板 (绝杀筑底期 whipsaw)
# ==================================================================================
...
FEE_RATE = 0.0010           # 5. 显式调仓双边总摩擦费率 (千分之 1 滑点)
ALPHA_SELL_BUFFER = 0.12    # 6. 防御减仓缓冲系数 (保持平滑减仓斩断高频摩擦)
...
print("🚀 正在启动《全局趋势共振感知 + 时变结构自适应滤波》终极演进战神资产配置系统...")
...
print("⚙️ 开始执行流式多机制感知非对称时变自适应组合测算...")
...
for i in range(len(test_months)-1):
    ...
    for day in day_chunks:
        ...
        # ==================================================================================
        # 🎯 核心数理创新：计算今日全截面的【全局贝塔趋势共振率 GBSR】
        # ==================================================================================
        total_active_etfs = len(day_probs)
        bullish_sync_count = 0
        for code in day_probs.keys():
            if code in day_env_dict:
                if day_env_dict[code]['Price'] > day_env_dict[code]['SMA']:
                    bullish_sync_count += 1
                    
        gbsr = bullish_sync_count / total_active_etfs if total_active_etfs > 0 else 0.5
        
        # 💡 机制一：让买入速度与大盘共振率平方强行解耦。牛市狂飙买入，筑底混沌市极度慢速试探
        dynamic_alpha_buy = 0.15 + 0.70 * (gbsr ** 2)
        
        # 💡 机制二：构建筑底期全局敞口防御锁。共振率低于 55% 的震荡筑底市，强制大幅压缩资金池最大载荷上限
        global_exposure_scaler = np.clip((gbsr - 0.35) / (0.65 - 0.35), 0.20, 1.0)
        ...
        if day_sent_z > SENT_BREAK_THRESHOLD:
            ...
            if bullish_assets:
                # 连续比例分配，乘以自适应全局防御锁敞口
                alloc = (1.0 / len(bullish_assets)) * global_exposure_scaler
                for code in bullish_assets:
                    target_weights[code] = alloc

        # 4.3 穿透变结构时变自适应滤波控制与精准扣费
        ...
        for code in all_codes:
            ...
            # 时变非对称因果律
            if w_tgt > w_prev:
                alpha = dynamic_alpha_buy  # 买入速度动态追随大盘气候
            else:
                alpha = ALPHA_SELL_BUFFER  # 卖出平滑保持低通滤除换手费
...
print("\n" + "="*112)
print("👑 真正大满贯战报：《9 只中证宽基 HGB》全局共振感知自适应滤波组合终极全摩擦实证")
print("================================================================================================================")
...
output_path = os.path.join(reports_dir, "ml_regime_adaptive_beast_report.csv")
perf_df.to_csv(output_path)
print(f"💾 斩断筑底期 Whipsaw、全时段完美降维暴杀基准的终极主权时序已成功导出：{output_path}\n")
