#!/usr/bin/env python3
"""自动 ETF 股票挖掘与测试循环 - 直到跑赢 baseline

这个脚本会循环执行：
1. 运行 lite_train 获取 ETF 预测
2. 使用 stock_deep_dive 深挖看涨股票
3. 运行 auto_tune_baseline 测试策略
4. 检查是否跑赢 baseline
5. 如果没跑赢，调整参数并重试

目标：找到一个跑赢 baseline 的 ETF 股票挖掘配置
"""
...
def check_baseline_performance(report_path: Path, min_excess: float = 0.0) -> tuple[bool, dict[str, Any]]:
    ...
    try:
        ...
        print(f"  {'🎉 跑赢 baseline!' if metrics['success'] else '😔 未跑赢 baseline'}")
...
def run_lite_train(config: dict[str, Any], force: bool = False) -> tuple[bool, Path]:
    ...
    success, _ = run_command(
        cmd,
        f"运行 lite_train (max_etfs={config['max_etfs_per_index']}, min_amount={config['min_amount']}, test_days={config['test_days']}, auto_tune={config.get('auto_tune', False)})",
        timeout=900,
    )
...
def run_deep_dive(config: dict[str, Any], force: bool = False) -> tuple[bool, Path]:
    ...
    success, _ = run_command(
        cmd,
        f"运行 deep_dive (top_etfs={config['top_etfs']}, stocks={config['top_stocks']})",
        timeout=1200,
    )
...
def run_auto_tune_baseline(start: str, end: str, min_excess: float = 0.0) -> tuple[bool, Path]:
    ...
    success, _ = run_command(
        cmd,
        f"运行 auto_tune_baseline (期间={start}~{end}, 最小超额={min_excess:.2%})",
        timeout=1800,
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="自动 ETF 股票挖掘与测试循环")
    ...
    for iteration in range(1, args.max_iterations + 1):
        ...
        if success:
            print(f"\n{'='*80}")
            print("🎊🎊🎊 成功！找到跑赢 baseline 的配置！")
            print(f"{'='*80}")
            print(f"✅ 迭代次数：{iteration}")
            print(f"✅ Lite Train 配置：{lite_config}")
            print(f"✅ Deep Dive 配置：{dive_config}")
    ...
    print(f"\n{'='*80}")
    print(f"😔 达到最大迭代次数 ({args.max_iterations})，未找到跑赢 baseline 的配置")
    print(f"{'='*80}")
    print(f"💡 建议：")
