...
def generate_time_splits(
    ...
    Returns:
        [(train_start, train_end, test_end), ...] 列表
    """
    df = df.copy()
...
def evaluate_time_split(
    ...
    return {
        ...
        "train_top_vs_all": train_metrics.get("top_vs_all", 0),
        ...
        "test_top_vs_all": test_metrics.get("top_vs_all", 0),
        ...
        "benchmark_outperform_rate": strategy_metrics.get("benchmark_outperform_rate", 0),
    }
...
def run_time_series_cv(
    ...
    # 遍历所有组合
    results = []
    total_combinations = len(splits) * len(l2_grid) * len(top_k_grid) * len(min_pred_grid)
    current = 0

    print(f"\n🚀 开始评估 {total_combinations} 个配置...\n")
    ...
    summary = {
        ...
        "std_test_rmse": results_df["test_rmse"].std(),

        # 策略指标的平均值
        "avg_strategy_return": results_df["strategy_cumulative_return"].mean(),
        "avg_strategy_sharpe": results_df["strategy_sharpe_like"].mean(),
        "avg_strategy_win_rate": results_df["strategy_win_rate"].mean(),

        # 策略指标的标准差
        "std_strategy_return": results_df["strategy_cumulative_return"].std(),
        "std_strategy_sharpe": results_df["strategy_sharpe_like"].std(),
...
def create_visualizations(
    ...
    write_horizontal_bar_chart(
        output_dir / "split_accuracy_comparison.svg",
        title="不同时间切分的测试集准确率",
    ...
    # 3. 策略收益随时间的变化
    split_return = results_df.groupby("split_idx")["strategy_cumulative_return"].mean()
    write_horizontal_bar_chart(
        output_dir / "split_strategy_return.svg",
        title="不同时间切分的策略收益",
        subtitle="评估策略在不同时期的盈利能力",
        rows=[(f"切分 {idx}", ret) for idx, ret in split_return.items()],
        value_kind="pct",
    )

    # 4. Top K 参数对胜率的影响
    topk_winrate = results_df.groupby("top_k")["strategy_win_rate"].mean()
    write_horizontal_bar_chart(
        output_dir / "topk_winrate.svg",
        title="Top K 参数对胜率的影响",
        subtitle="平均交易胜率",
        rows=[(f"Top {k}", wr) for k, wr in topk_winrate.items()],
        value_kind="pct",
    )
...
def main() -> int:
    ...
    parser.add_argument("--data-dir", type=Path, default=ROOT / "data" / "topic2_broad_base")
    parser.add_argument("--sentiment", type=Path, default=ROOT / "data" / "alternative_data" / "google_trends_sentiment.csv")
    parser.add_argument("--output", type=Path, default=REPORTS / "time_series_cv_results.csv")
    parser.add_argument("--summary", type=Path, default=REPORTS / "time_series_cv_summary.json")
    parser.add_argument("--charts", type=Path, default=REPORTS / "charts" / "time_series_cv")
    parser.add_argument("--n-splits", type=int, default=5, help="时间切分数量")
    parser.add_argument("--min-train-days", type=int, default=180, help="最小训练天数")
    parser.add_argument("--test-days", type=int, default=60, help="测试天数")
    parser.add_argument("--target", default="fwd_ret_5", help="目标列名")
